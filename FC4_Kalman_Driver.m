...
A = A([1:6,8:end],[1:6,8:end]);
B = B([1:6,8:end],[301,2107:2112,2195,2196,3943:3954]);
C = C(:,[1:6,8:end]);
...
% filtered_measurements = lowpass(system_measurements,6,f_sample);
filtered_measurements = system_measurements;
filtered_measurements = [zeros(714,6);filtered_measurements];
...
kalman_dir = 'C:\Umaine Google Sync\Masters Working Folder\FOCAL_C2\Models\FOCAL_C4\Linear_Files\5 - Kalman Files';

% load(sprintf('%s\\FC4_P.mat',kalman_dir),'P');
% load(sprintf('%s\\FC4_Q.mat',kalman_dir),'Q');
% Q = 2*Q;
...
for i = 1:length(ss_time)-1
    ...
    Y(:,i) = C_platform*x + D_platform*u_platform;
end
