...
class PreProcess():
    ...
    def series_to_supervised(self, data, wind_var_number, wave_var_number, n_in=1, n_out=1,
        ...
        agg.columns = names
        print(agg)
        print("Aggregated df shape:", agg.shape)
        # drop rows with NaN values
        if dropnan:
            agg.dropna(inplace=True)
            # After dropping NaN values
            print("Shape after dropping NaNs:", agg.shape)
        ...
        if wind_predictor:
            # Before and after interpolating wind data (if applicable)
            print("Shape before wind interpolation:", agg.shape)
            ...
            print("Shape after wind interpolation:", agg.shape)
        # Interpolate wave future data if needed
        if wave_predictor:
            # Before and after interpolating wave data (if applicable)
            print("Shape before wave interpolation:", agg.shape)
            # Define interpolation points
            x = np.linspace(0, n_out - 1, n_in)
            ...
            # Wave interpolation code...
            print("Shape after wave interpolation:", agg.shape)
    ...
    def dynamic_sig_char(self, chkpnt_no, significant_length):
        # Check that significant_length_idx is within bounds
        time_diff = self.dataset['Time'].iloc[1] if len(self.dataset) > 1 else 0
        significant_length_idx = int(significant_length / time_diff) if time_diff else 0
        print(f"Calculated significant_length_idx: {significant_length_idx}, Dataset length: {len(self.dataset)}")

        if significant_length_idx >= len(self.dataset):
            print("significant_length_idx is out of bounds.")
            return
        ...
        # Ensure checkpoints are within the valid range
        ...
        for i, chkpnt in enumerate(chkpnts):
            ...
            if chkpnt - significant_length_idx < 0 or chkpnt >= len(self.dataset):
                print(f"Index {chkpnt} is out of bounds.")
                continue
            
            time_slice = np.array(self.dataset['Time'].iloc[chkpnt - significant_length_idx:chkpnt])
            wave_slice = np.array(self.dataset['waveStaff5'].iloc[chkpnt - significant_length_idx:chkpnt])
            mean_wave = np.mean(wave_slice)
            
            T, H, _, _ = self.zero_crossing(time_slice, wave_slice - mean_wave)
