...
from Fatigue_Estimation.fatigue_damage_RUL import RUL_class
...
class CombinedController:
    ...
    def wfc_controller(self, id, current_time, measurements):
        ...
        setpoints = self.calculate_setpoints(current_time, measurements)
        return setpoints

    def calculate_setpoints(self, current_time, measurements):
        ...
        HorWindV = measurements["HorWindV"]
        VS_GenPwr = measurements["VS_GenPwr"]
        if y_hat_raw is not None and FOWT_pred_state == 'BlPitchCMeas':
            self.publish_prediction(Pred_B + BlPitchCMeas_pred_error, t_pred, current_time, present_state_web, time_horizon, Pred_Delta_B, RotSpeed, HorWindV, VS_GenPwr)
        else:
            self.publish_prediction(Pred_B, t_pred, current_time, present_state_web, time_horizon, Pred_Delta_B, RotSpeed, HorWindV, VS_GenPwr)
    ...
    def publish_prediction(self, Pred_B, t_pred, current_time, present_state_web, time_horizon, Pred_Delta_B, RotSpeed, HorWindV, VS_GenPwr):
        Pred_Delta_B = Pred_Delta_B * 180 / np.pi
        current_time = current_time - time_horizon - 1
        self.pub_socket.send_json({"Pred_B": Pred_B, "t_pred": t_pred, "present_state_web": present_state_web, "current_time": current_time,"Pred_Delta_B": Pred_Delta_B, "RotSpeed": RotSpeed, "HorWindV": HorWindV,"VS_GenPwr": VS_GenPwr})
        if current_time % 10 == 0:
            print(f"Published prediction updates: Pred_B: {Pred_B}, t_pred: {t_pred}, present state: {present_state_web}, current time: {current_time}, Pred_Delta_B: {Pred_Delta_B}, RotSpeed: {RotSpeed}, HorWindV: {HorWindV}, VS_GenPwr: {VS_GenPwr}")
