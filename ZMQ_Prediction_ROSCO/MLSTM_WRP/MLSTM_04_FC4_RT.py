...
def run_MLSTM(start_simulation, end_simulation, timestep, TEST_NUM, MODEL_PATH, SCALER_PATH, data_frame_inputs, TIME_HORIZON):
    ...
    dummy_file = "ZMQ_Prediction_ROSCO/MLSTM_WRP/Data/FC4/windwave_test.csv"
    ...
    print(f"Starting simulation with T size: {len(T)}")
    
    for i, t in enumerate(T):
        print(f"\nIteration: {i}, Time: {t}")
        print(f"Expected operations for time: {t}")
        y, yhat, idx = get_data_for_frame(t, data, model, dof, n, m, waveshift, scaler_b4_super, scaler, wavedata, timestep, df, input_columns, num_features)  # Now capturing idx
        print(f"Accessing y and yhat with idx: {idx}, n: {n}, m: {m}, waveshift: {waveshift}")
        print(f"y = {y[i]}\n yhat = {yhat[i]}")
...
def get_data_for_frame(t, data, model, dof, n, m, waveshift, scaler_b4_super, scaler, wavedata, timestep, df, input_columns, num_features):
    print(f"\nCalled get_data_for_frame for t = {t}s")
    idx = np.abs(data.dataset['Time'] - t).argmin()
    print(f"Closest index in dataset for time {t}: {idx}")
    y_size = waveshift
    print(f"y_size calculated as: {y_size}")
    idx = np.abs(data.dataset['Time'] - t).argmin()
    y_size = waveshift
